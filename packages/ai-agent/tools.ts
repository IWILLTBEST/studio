...
/** 写回 IR（.ir.json 校验 JSON；.uixml 做首标签探针——完整校验交给 ir2eez 编译）*/
export function writeIr(ctx: ToolContext, content: string): string {
    const p = fullIrPath(ctx);
    if (p.endsWith(".uixml") || p.endsWith(".xml")) {
        // XML surface syntax: cheap well-formedness probe, real validation at compile
        const probe = content.trimStart();
        if (!probe.startsWith("<")) {
            throw new Error("UIXML 内容必须以 < 开头（XML）");
        }
    } else {
...
export async function createProject(
    ...
    const template = {
        ...
        settings: {
            ...
            build: {
                // configurations 必须至少有一个，Run/debug 的 build 分支会取
                // configurations[0].name（无守卫时崩 Module build error）
                configurations: [{ name: "Default" }]
            }
        },
        themes: [{ name: "Default", colors: [] }],
        // lvglStyles 必须写：LVGL 部件属性面板/预览/构建都会直接访问
        // project.lvglStyles.defaultStyles/allStyles，缺失时渲染崩
        lvglStyles: { styles: [] },
        fonts: [],
        bitmaps: [],
        userPages: [
