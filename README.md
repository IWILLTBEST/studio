...
| GIF | — (fork-only, see [#758](https://github.com/eez-open/studio/issues/758)) | fork | `lv_gif_create` + `lv_gif_set_src` with raw GIF bytes embedded as an `lv_img_dsc_t`; requires `LV_USE_GIF=1` on the device; requires `LV_USE_GIF=1` on the device; the fork ships editor wasm rebuilt with `LV_USE_GIF` for 9.2.2/9.3.0/9.4.0/9.5.0 (upstream 8.4.0 already has it), so the animation plays in Run mode; a per-widget *From file system* switch streams the GIF from the device file system instead (`lv_gif_set_src(obj, "S:/ui_image_x.gif")`, the .gif file is copied next to the generated sources, path prefix from the build *File system path* setting), or points verbatim at an existing device file via the optional *File* property (nothing is emitted - Source then serves only as the editor preview stand-in) |
