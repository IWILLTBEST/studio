...
| GIF | — (fork-only, see [#758](https://github.com/eez-open/studio/issues/758)) | fork | `lv_gif_create` + `lv_gif_set_src` with raw GIF bytes embedded as an `lv_img_dsc_t`; requires `LV_USE_GIF=1` on the device; requires `LV_USE_GIF=1` on the device; the fork ships an editor wasm rebuilt with `LV_USE_GIF` for 9.4.0 (plus upstream 8.4.0 which has it), so the animation plays in Run mode; versions without a gif-enabled wasm fall back to a placeholder |
